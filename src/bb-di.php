...
$di['is_client_logged'] = function() use($di) {
    if(!$di['auth']->isClientLoggedIn()) {
        throw new Exception('<p>Unfortunately, your session has timed out. Please <a href=/login>login</a> to your account again, before attempting to continue your previous activity.</p>');
    }
    return true;
};
...
$di['is_admin_logged'] = function() use($di) {
    if(!$di['auth']->isAdminLoggedIn()) {
        throw new Exception('<p>Unfortunately, your session has timed out. Please <a href=/bb-admin>login</a> to your account again, before attempting to continue your previous activity.</p>');
    }
    return true;
};
